...
| 模式 | 命令 | 作用 |
|------|------|------|
| **patch** | `fuck-openclaw-safe patch` | 改写 dist 文件中的硬编码限制 |
| **config** | `fuck-openclaw-safe config` | 生成 `openclaw.json` 启用 48 个被禁功能 |
| **full** | `fuck-openclaw-safe full` | 两者同时执行（**推荐**） |
...
- `fuck-openclaw-safe unpatch` 立即恢复
...
**怎么撤销？** `fuck-openclaw-safe unpatch` 一键恢复。
