...
import { H as VERSION, W as loadDotEnv } from "./config-irA2OYte.js";
...
import { u as installUnhandledRejectionHandler } from "./runner-CMatzMlG.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-DY2TBqBE.js";
import { t as assertSupportedRuntime } from "./runtime-guard-Cs_ClFhP.js";
import { t as emitCliBanner } from "./banner-BFMgZqYZ.js";
import "./doctor-config-flow-DsZFcQEG.js";
import { n as ensureConfigReady } from "./config-guard-CPqnbk9c.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-C5fIqN53.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BsMO1u1v.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-HpSWDV9C.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-BPR9ZDHy.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-C71KenfK.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-D4U9twzG.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-D4U9twzG.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-C6z9MZ_t.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-C6z9MZ_t.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-xlD3i_8e.js");
	...
	if (primary) {
		...
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-BFiKAp7V.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-BzJoB0Qs.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { loadConfig } = await import("./config-irA2OYte.js").then((n) => n.t);
